...
- `configuration.nix` — Single-file declarative system config (~600 lines). All services, packages, networking, and users defined here.
- `hardware-configuration.nix` — **Auto-generated by `nixos-generate-config`. Do not edit manually.**
- `nix-functions.zsh` — Nix convenience shell functions (`nsp`, `nss`, `nwp`), sourced by `interactiveShellInit` in `configuration.nix`
