...
Provision the images directory for UID 33 and the backup directory for
`${UID}:${GID}` before starting. Bind mounts use `create_host_path: false` so
missing directories fail rather than silently become root-owned.
