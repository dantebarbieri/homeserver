...
# drives everything from the Pi's local /home/openclaw/HomeServer clone.
#
...
ssh "$PI_HOST" 'sudo -u openclaw bash -c "
  cd /home/openclaw/HomeServer
  git fetch --quiet
  if ! git diff --quiet HEAD origin/main -- pi/; then
    echo \"REPO AHEAD — run: sudo -u openclaw git -C /home/openclaw/HomeServer pull --ff-only\" >&2
    exit 11
  fi
  git -C /home/openclaw/HomeServer log --oneline -1
"'

echo "[2/5] apply pending sqlite migrations…" >&2
ssh "$PI_HOST" 'sudo -u openclaw python3 \
  /home/openclaw/HomeServer/pi/apply-migrations.py \
  /var/lib/openclaw/state.db \
  /home/openclaw/HomeServer/pi/sqlite-migrations/'

echo "[3/5] install workspace files (TRAVEL.md + skills/)…" >&2
ssh -t "$PI_HOST" 'sudo -u openclaw bash -c "
  set -e
  WORKSPACE=/home/openclaw/.openclaw/workspace
  SOURCE=/home/openclaw/HomeServer/pi/openclaw-workspace
...
ssh -t "$PI_HOST" 'sudo -u openclaw bash -c "
  ...
  SOURCE=/home/openclaw/HomeServer/pi/openclaw-scripts
  mkdir -p \"\$SCRIPTS\"
...
ssh -t "$PI_HOST" 'sudo -u openclaw python3 \
  /home/openclaw/HomeServer/pi/openclaw-jobs/upsert-jobs.py \
  /home/openclaw/.openclaw/cron/jobs.json \
  /home/openclaw/HomeServer/pi/openclaw-jobs/travel-jobs.json'
